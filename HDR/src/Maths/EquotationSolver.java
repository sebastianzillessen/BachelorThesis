...
public class EquotationSolver {
     ...
     * @return
     */
    public static Vector solve(AbstractMatrix A, Vector b, EquotationSolverAlgorithm algorithm) throws EquotationSolverException {
        if (algorithm == EquotationSolverAlgorithm.LU) {
            return solveWithLU(A, b);
        } else if (algorithm == EquotationSolverAlgorithm.SOR) {
     ...
     * @return the vector x.
     */
    private static Vector solveWithLU(AbstractMatrix a, Vector b) throws EquotationSolverException {
        if (!a.isQuadratic())
            throw new EquotationSolverException("Matrix A is not quadratic.", a, b);
        if (b.length() != a.rows())
            throw new EquotationSolverException("Size of Vextor and Matrix does not match.", a, b);
        if (!a.isPentadiagonale())
            throw new EquotationSolverException("Matrix A is not pentadiagonale.", a, b);
    ...
    private static Vector solveWithSOR(AbstractMatrix a, Vector b) throws EquotationSolverException {
        if (!a.isQuadratic())
            throw new EquotationSolverException("Matrix A is not quadratic.", a, b);
        if (b.length() != a.rows())
            throw new EquotationSolverException("Size of Vextor and Matrix does not match.", a, b);
        if (!a.isSymmetric())
            throw new EquotationSolverException("Matrix A is not symmetric.", a, b);
        if (!a.isPositiveSemiDefinit())
            throw new EquotationSolverException("Matrix A is not positive-semi-definite.", a, b);

        if (!(a instanceof BandMatrix))
            throw new EquotationSolverException("Matrix A is no Band-Matrix. SOR-Solver should only be used on them.", a, b);
        return solveSORBand((BandMatrix) a, b);
    }


    protected static Vector forwardElimination(Vector b, AbstractMatrix L) {
    ...
    protected static Vector backwardSubstitution(AbstractMatrix U, Vector y) {
        int n = y.length();
    ...
    protected static Vector solveSORBand(BandMatrix a, Vector b) {
